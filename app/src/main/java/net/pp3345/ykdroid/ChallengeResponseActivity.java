 ...
 * <code>"net.pp3345.ykdroid.intent.action.CHALLENGE_RESPONSE"</code> intent to
 * send a challenge
 * to a YubiKey and receive the response.
 * <p>
 * The challenge must be passed in an extra <code>byte[] challenge</code>. Upon
 * successful completion,
 * the activity returns an extra <code>byte[] response</code> in the result
 * intent. Optionally,
 * an extra <code>String purpose</code> may be passed in the intent to identify
 * the purpose of the
 * challenge. ykDroid will use this identifier to remember and pre-select the
 * slot used for each
 * purpose.
 * </p>
 */
public class ChallengeResponseActivity extends Activity implements ConnectionManager.YubiKeyConnectReceiver,
        ConnectionManager.YubiKeyUsbUnplugReceiver, AdapterView.OnItemSelectedListener {
    ...
    public void onYubiKeyConnected(final YubiKey yubiKey) {
        ...
        final AsyncTask<Void, Void, byte[]> challengeResponseTask = new AsyncTask<Void, Void, byte[]>() {
            ...
            protected byte[] doInBackground(final Void... nothing) {
                try {
                    return yubiKey.challengeResponse(ChallengeResponseActivity.this.selectedSlot,
                            ChallengeResponseActivity.this.challenge);
            ...
            protected void onPostExecute(final byte[] bytes) {
                ...
                if (this.executionException == null) {
                    ...
                    ChallengeResponseActivity.this.connectionManager
                            .waitForYubiKeyUnplug(ChallengeResponseActivity.this);
                    ChallengeResponseActivity.this.showError();

                    ((TextView) ChallengeResponseActivity.this.findViewById(R.id.info))
                            .setText(R.string.unplug_yubikey);
                }
            }
        };
        this.findViewById(R.id.slotSelection).setVisibility(View.GONE);
